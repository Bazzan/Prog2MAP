...
public class ImagePanel extends JPanel {
	...
	public List<Place> placesMarked = new LinkedList<>();
		...
		public void mouseClicked(MouseEvent mev) {
			...
			if (SwingUtilities.isRightMouseButton(mev)) {
				if (place instanceof NamePlace) {
					...
					String message = place.getName() + ": " + ((DescribedPlace) place).getDescription() + "'"
							+ place.getCordinates() + "'";
					JOptionPane.showMessageDialog(null, message, "Place Information!", JOptionPane.OK_OPTION);
	...
	public List<Place> removeMarked() {
		...
		List<Place> removeMarkedPlaces = new LinkedList<Place>(placesMarked);
		placesMarked.clear();
		this.repaint();
